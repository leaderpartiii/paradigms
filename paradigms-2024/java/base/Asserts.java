...
public final class Asserts {
    ...
    public static void assertEquals(final String message, final Object expected, final Object actual) {
        final String reason = "%s:%n     expected `%s`,%n       actual `%s`"
                .formatted(message, toString(expected), toString(actual));
    ...
    public static void assertEquals(final String message, final double expected, final double actual, final double precision) {
        assertTrue(
                "%s: Expected %.12f, found %.12f".formatted(message, expected, actual),
    ...
    public static void assertSame(final String message, final Object expected, final Object actual) {
        assertTrue("%s: expected same objects: %s and %s".formatted(message, expected, actual), expected == actual);
    }
    ...
    public static AssertionError error(final String format, final Object... args) {
        final String message = format.formatted(args);
